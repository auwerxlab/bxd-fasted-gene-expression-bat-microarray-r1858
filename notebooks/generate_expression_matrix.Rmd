...
run.metadata.url <- "https://sv-renku-git.epfl.ch/api/v4/projects/lisp%2Fbxd-fasted-knowledge-base/repository/files/data%2Fruns%2Fmicroarray_runs.txt/raw?ref=5709a4b7"
...
Also download the run samples metadata
```{r download_samples_metadata, eval=F}
sample.metdata.url <- read.table(here("data/microarray_runs.txt"), sep ="\t", header = 1) %>%
  filter(run_id == run.id) %>%
...
system(paste0("wget https://media.affymetrix.com/analysis/downloads/lf/wt/MoGene-2_0-st/MoGene-2_0-st_rev1.zip -P ",
              here("data/platform_files", platform.id)))
system(paste0("cd ", here("data/platform_files", platform.id),
              " && unzip MoGene-2_0-st_rev1.zip",
              " && rm MoGene-2_0-st_rev1.zip",
              " && mv CD_MoGene-2_0-st_rev1/Full/MoGene-2_0-st/LibFiles/* ./",
              " && rm -r CD_MoGene-2_0-st_rev1",
              " && gzip *"))
```
